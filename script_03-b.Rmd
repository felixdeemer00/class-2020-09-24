...
sep_desc <- left_join(sep_old, sep_new, 
...
sep_asc <- left_join(sep_old, sep_new, 
                   by = c("id","title", "name", "department"), 
                   suffix = c("_old","_new")) %>%
  mutate(u_grad_diff = u_grad_new - u_grad_old) %>%
  arrange((u_grad_diff)) %>%
...
gt(sep_asc) %>%
  tab_header(title = "Courses with Greatest")
...
sep_5 <- rbind(sep_asc,sep_desc)
  
sep_5 %>%
  ggplot(aes(x = fct_reorder(title, u_grad_diff), y = u_grad_diff)) +
  geom_col() +
  coord_flip() +
  labs(x = "Class", "Change in Enrollment", 
       "Courses with Greatest Increase or Decrease in Enrollment")
